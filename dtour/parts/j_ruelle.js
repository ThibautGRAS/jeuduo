...
Object.assign(RuelleVue, {
  dessinerHud(){
    ...
    /* Le chargeur : une couronne de FINS bâtonnets, plus une bague
       lumineuse qui la double. Des segments épais faisaient camembert ;
       fins et nombreux, ça fait cadran. */
    const rInt = zt.r * 1.10, rExt = zt.r * 1.30;
    const nSeg = Math.max(arme.chargeur, 12);
    const remplis = Math.round(nSeg * (h.balles / arme.chargeur));
    for (let k = 0; k < nSeg; k++){
      const pas2 = 6.2832 / nSeg, marge = pas2 * 0.30;
      const a0 = -Math.PI / 2 + k * pas2 + marge, a1 = a0 + pas2 - marge * 2;
      ctx.beginPath();
      ctx.arc(zt.x, zt.y, rExt, a0, a1);
      ctx.arc(zt.x, zt.y, rInt, a1, a0, true);
      ctx.closePath();
      ctx.fillStyle = k < remplis ? "#F5C13A" : "rgba(240,240,250,.16)";
      ctx.fill();
    }
    /* la bague lumineuse : trois traits de plus en plus larges et pâles,
       c'est ce qui fait la lueur sans shadowBlur */
    for (const [ep, al2] of [[zt.r * 0.10, 0.10], [zt.r * 0.055, 0.20], [zt.r * 0.022, 0.60]]){
      ctx.strokeStyle = "rgba(247,193,58," + al2 + ")";
      ctx.lineWidth = ep;
      ctx.beginPath(); ctx.arc(zt.x, zt.y, zt.r * 1.02, 0, 6.2832); ctx.stroke();
    }
    /* La balle, plus haut, et le mot TIR dessous : l'un se lit de loin,
       l'autre dit ce que ça fait. */
    ctx.save();
    ctx.translate(zt.x, zt.y - zt.r * 0.16); ctx.rotate(-0.62);
    const bl = zt.r * 0.72, bw = zt.r * 0.26;
    const gb = ctx.createLinearGradient(-bw / 2, 0, bw / 2, 0);
    gb.addColorStop(0, recharge ? "rgba(150,142,124,.5)" : "#8F6A1E");
    gb.addColorStop(0.42, recharge ? "rgba(210,202,182,.5)" : "#FCE39B");
    gb.addColorStop(1, recharge ? "rgba(150,142,124,.5)" : "#A87C22");
    ctx.fillStyle = gb;
    ctx.beginPath();
    ctx.moveTo(-bw / 2, bl * 0.44); ctx.lineTo(bw / 2, bl * 0.44);
    ctx.lineTo(bw / 2, -bl * 0.06);
    ctx.quadraticCurveTo(bw / 2, -bl * 0.44, 0, -bl * 0.52);
    ctx.quadraticCurveTo(-bw / 2, -bl * 0.44, -bw / 2, -bl * 0.06);
    ctx.closePath(); ctx.fill();
    ctx.fillStyle = recharge ? "rgba(120,114,100,.5)" : "#B98C2E";
    ctx.fillRect(-bw / 2, bl * 0.12, bw, bl * 0.32);
    ctx.restore();
    ctx.textAlign = "center"; ctx.textBaseline = "middle";
    ctx.font = "800 " + Math.round(zt.r * 0.34) + "px 'Baloo 2', system-ui, sans-serif";
    ctx.fillStyle = recharge ? "rgba(255,255,255,.45)" : "#FFF";
    ctx.fillText(recharge ? "…" : "TIR", zt.x, zt.y + zt.r * 0.48);
    /* le compte, sous le bouton : un chiffre vaut mieux qu'un décompte
       de bâtonnets quand il en reste peu */
    ctx.font = "800 " + Math.round(zt.r * 0.26) + "px 'Baloo 2', system-ui, sans-serif";
    ctx.fillStyle = "rgba(247,193,58,.92)";
    ctx.fillText(h.balles + "/" + arme.chargeur, zt.x, zt.y + zt.r * 1.62);

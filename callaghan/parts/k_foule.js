...
const FOULE_PHRASES = {
  ...
  fin_de_soiree: [
    ...
    "J'suis pas bourré, j'suis fatigué.",
    "Merde, j'ai dit quoi ?",
  ...
  kevin:    ["C'est ma tournée. Enfin, presque.", "Jägerbombs. Point.",
             "On fait pas les cons. On fait les Jäger."],
  ...
  gautier:  ["Ce soir je joue pas. Enfin, presque pas.",
             ...
             "Mon frère me surveille. Il surveille tout.",
             "Je cours demain. Enfin, dimanche."],
  capucine: ["GOGO ! Tu fais quoi ?",
             "Ce bar aurait besoin d'un autre éclairage.",
             "Ces chaises, non. Vraiment non.",
             "On rentre mardi. Ou pas."],
  bobito:   ["Chaussette a mangé une chaussette.",
             "Mes troisièmes ignorent le prétérit.",
             "Il est où, Rémy ? Il danse pas.",
             "On en reprend un et on danse."],
...
const FOULE_DISCUSSIONS = {
  ragots: [
    ...
    ["Rémy dit Bobito à tout bout de champ.", "C'est sa copine.",
     "Je sais, mais à CHAQUE phrase."],
    ["Bobito a ramené des photos du chien.", "Encore ?", "Trente-quatre."],
    ["Le chien s'appelle Chaussette.", "C'est mignon.",
     "Attends de voir ce qu'il fait aux chaussettes."],
    ["Bobito est prof d'anglais.", "Elle corrige nos fautes ?",
     "Que celles en anglais. Pour le reste elle a renoncé."],
    ["Elle danse depuis tout à l'heure.", "Bobito ?", "Bobito.",
     "Elle s'arrêtera pas."],
    ["Rémy essaie de suivre.", "En danse ?", "En danse.",
     "Il devrait pas."],
    ["Bobito a dit un truc en anglais.", "Et ?",
    ...
    ["Il vit à Malte, maintenant.", "Pour le soleil ?",
     "Pour le poker.", "Ah. Pour le soleil, alors."],
    ["Capucine l'appelle Gogo.", "Devant tout le monde ?",
     "Surtout devant tout le monde."],
    ["Gautier a gagné combien hier ?", "Il dit qu'il a gagné.",
     "Donc il a perdu."],
    ["Capucine refait des appartements.", "Le sien ?",
     "Ceux des autres. Le sien, c'est un chantier."],
    ["Gautier court un semi dimanche.", "Après ce soir ?",
     "Après ce soir.", "Il court, ou il rentre en courant ?"],
    ["Thibaut surveille son frère.", "Il fait quoi ?",
     "Il compte ses verres. À voix haute."],
    ["Ils repartent quand ?", "Mardi.", "Ils disent toujours mardi."],
    ["Capucine a redécoré leur salon trois fois.", "En un an ?",
     "En six mois. Gogo n'a rien remarqué."],
  ...
  /* CE QU'ILS DISENT QUAND IL EST TARD. Ça jure, c'est un bar — mais la
     ...
     de lui-même est drôle ; un ivrogne qui se moque du voisin est juste
     un type désagréable, et on l'entend quinze fois par soirée. */
  ivres: [
    ...
    ["Putain la marche.", "Y'a pas de marche.", "Ben si, je viens de la faire."],
    ["J'ai la tête dans le cul.", "Ça se voit un peu.", "Ça se VOIT ?"],
    ["Merde, mon téléphone.", "Il est dans ta main.",
     "Ah. Merde quand même."],
    ["J'suis pas bourré.", "Personne a dit ça.",
     "Ben voilà. Alors j'suis pas bourré."],
    ["C'est quoi ce bordel dans mon verre ?", "C'est ce que tu as commandé.",
     "J'ai des goûts de merde."],
    ["J'ai essayé de la draguer.", "Et ?",
     "J'ai parlé de la Formule 1. Vingt minutes."],
    ["J'ai dit un truc con tout à l'heure.", "Lequel ?",
     "Tous, en fait."],
    ["Je vais me coucher.", "Il est 22 h.", "Putain déjà ?", "DÉJÀ ?"],
    ["Faut que j'arrête.", "D'accord.", "Pas maintenant. Le mois prochain."],
    ["J'me suis pris les pieds dans rien.", "Dans rien du tout ?",
     "Dans le vide. Le vide entier."],
    ["Franchement, on est cons.", "Ouais.", "Mais bien."],
    ["J'ai chanté ?", "Tu as chanté.", "Fort ?", "Très fort.", "Merde."],
    ["Je paie ma tournée.", "Avec quoi ?",
     "Avec ma carte. Elle est chez moi. Ma carte est chez moi."],
    ["T'as vu comment je danse ?", "Oui.", "C'est bien, hein ?",
     "Non. Mais continue."],
  ],
  commun: [
...
/* CE QUE FAIT UN HABITUÉ QUAND IL NE FAIT RIEN. Ils étaient debout,
   immobiles, à la même place toute la soirée : une haie d'honneur, pas un
   bar. Trois états s'ajoutent au repos, et chacun a sa condition —
   sinon tout le monde danse tout le temps et plus rien ne veut dire rien.

     `danse`   quand l'ambiance est HAUTE. C'est la récompense visible
               d'une bonne tournée : la salle se met à bouger.
     `titube`  quand le champion lui-même est pompette, ou de temps en
               temps. Un bar où personne ne tangue est un bar fictif.
     `assis`   sur un tabouret libre du comptoir, pour un long moment.
...
/* La cadence des pas de danse et du tangage. Plus lente que la marche :
   deux images qui alternent trop vite se lisent comme un tremblement. */
const FOULE_CADENCE_DANSE = 3.4;
/* LA TAILLE D'UN HABITUÉ ASSIS. Il quitte le premier plan pour la ligne
   des tabourets, plus haute, donc plus loin : il rapetisse. Mais pas de
...
/* IL S'ASSOIT À CÔTÉ, PAS DESSUS. Le tabouret est dessiné AVANT la foule,
   ...
   sur rien. Décalé d'un centième de monde vers la droite, le tabouret
   réapparaît à sa gauche et la scène se lit. */
const FOULE_ASSIS_DECALE = 0.013;
...
Object.assign(Tournee, {
  ...
  composerFoule(){
    ...
    this.echange = null;
    this.humeurT = melange(FOULE_HUMEUR[0], FOULE_HUMEUR[1], Math.random());
  ...
  majFoule(dt){
    ...
    for (let i = this.foule.length - 1; i >= 0; i--){
      ...
      m.t += dt;
      /* danse et tangage : ils restent sur place, seule la cadence court */
      if (m.etat === "danse" || m.etat === "titube"){
  ...
  /* La pose d'un habitué se déduit de son état, comme partout ailleurs —
     elle était écrite dans la VUE, ce qui la rendait impossible à faire
     grandir sans toucher au dessin.

     Chaque pose se replie sur `idle` si la planche du personnage ne l'a
     pas encore : les onze habitués n'auront pas leurs poses le même jour,
     et le bar ne doit pas attendre le dernier. */
  poseFoule(m){
    const p = m.ref.prefixe, dispo = n => Images.table[p + "_" + n] ? n : null;
    /* LE DERNIER REPLI EST UN NOM, PAS UNE RECHERCHE. `dispo("idle")`
  ...
  /* Un habitué change d'humeur : il se met à danser, il tangue, ou il va
     s'asseoir. Il ne le fait que depuis sa grappe — on ne s'assoit pas au
     milieu d'une traversée. */
  majHumeurs(dt){
    this.humeurT -= dt;
    ...
    this.humeurT = melange(FOULE_HUMEUR[0], FOULE_HUMEUR[1], Math.random());
    const dispos = this.foule.filter(m => m.etat === "grappe");
